...
LeoBuild.prototype.build = function(dir,callback){
  ...
  if (!project) {
    out.warn('Cannot not find any files to build in ' + dir + '. Skipping');
    return callback();
  }
...
function preprocFile(next){
  ...
  if(ret instanceof Error){
    out.error('Sketch "' + this.name + '" does not exist.');
    return next(new Error('Sketch "' + this.name  + '" does not exist.'));
  }
...
function compileFile(next){
  ...
  env.object_file = this.toBuildDir(toO(this.name));
  var cmd = func(env);
  task(cmd,next);
}
